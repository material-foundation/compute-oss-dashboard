...
if [ ! -f "$CONFIG_PATH" ]; then
  ...
  echo "A standard one has been made for you based off config.sh.template."
  echo "Please configure $CONFIG_PATH to your preferences."

  mkdir -p "$CONFIG_ROOT_PATH"
  cp $DIR/../config.sh.template "$CONFIG_PATH"
  chmod +x $CONFIG_PATH
...
gcloud_attach_disk() {
  gcloud_project compute instances attach-disk $COLLECTOR_VM_NAME "$@" --zone "$ZONE"
}
...
open_ssh() {
  export PORT="22"
  if [ ! -z "$(gcloud_instances describe $COLLECTOR_VM_NAME --zone=$ZONE | grep "ssh-222")" ]; then
    PORT="222"
  fi
...
remote_exec() {
  ...
  gcloud --project=${PROJECT} compute ssh $COLLECTOR_VM_NAME --zone $ZONE --ssh-flag="-p $PORT" --command "$1" || exit 1
}

copy_from_local_to_remote() {
  gcloud --project=${PROJECT} compute copy-files "$1" "$COLLECTOR_VM_NAME:$2" --zone $ZONE || exit 1
}
